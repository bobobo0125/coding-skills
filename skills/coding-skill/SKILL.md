...
在执行编码任务时，会自动读取项目根目录下的规则文件：

| 规则文件 | 说明 | 读取时机 |
|---------|------|---------|
| `wiki/架构规范.md` | 项目特定的架构约束 | 始终读取 |
| `wiki/编码规则.md` | 项目特定的代码风格约定 | 始终读取 |
| `wiki/技术栈规范.md` | 技术选型和依赖规范 | 始终读取 |

**读取逻辑**：
1. 首先读取项目根目录下的 `wiki/` 目录
2. 如果存在上述规则文件，加载其内容作为额外约束
3. 如果规则文件不存在，跳过该规则（使用默认规范）
4. 项目特定规则优先于通用规范
...
本技能生成的文档统一存放在项目根目录的 `doc/` 目录下：

| 文档类型 | 输出路径 |
|---------|---------|
| 技术方案 | `doc/<功能名>-技术方案.md` |
| 设计文档 | `doc/<功能名>-设计.md` |
| 接口文档 | `doc/<功能名>-接口.md` |
