...
在执行代码审查时，会自动读取项目根目录下的规则文件：

| 规则文件 | 说明 | 读取时机 |
|---------|------|---------|
| `wiki/CR规则.md` | 项目特定的代码审查标准 | 始终读取 |
| `wiki/架构规范.md` | 项目架构约束 | 始终读取 |
| `wiki/编码规则.md` | 代码风格约定 | 始终读取 |

**读取逻辑**：
1. 首先读取项目根目录下的 `wiki/` 目录
2. 如果存在上述规则文件，加载其内容作为审查要点
3. 如果规则文件不存在，使用通用审查标准
4. 项目特定审查规则会作为额外检查项
...
审查过程中生成的文档统一存放在项目根目录的 `doc/` 目录下：

| 文档类型 | 输出路径 |
|---------|---------|
| 审查报告 | `doc/<功能名>-审查报告.md` |
